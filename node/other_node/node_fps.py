...
class Node(DpgNodeABC):
    ...
    def add_node(
        ...
        small_window_w = self._setting_dict["input_window_width"]
            ...
            if connection_type == self.TYPE_TIME_MS:
                ...
                if input_value != "":
                    ...
                    if source_tag not in self._value_history:
                        self._value_history[source_tag] = deque(maxlen=self._buffer_len)
                        self._value_history[source_tag].append(input_value)
                    else:
                        self._value_history[source_tag].append(input_value)

                    # 平均処理時間
                    average_elapsed_time = sum(self._value_history[source_tag]) / len(
                    ...
                    text += " (" + "{:.0f}".format(input_value).zfill(4) + "ms)"
    ...
    def _add_slot(self, sender, data, user_data):
        ...
        if self._max_slot_number > self._slot_id[tag_node_name]:
            ...
            tag_node_inputXX_value_name = (
                tag_node_name + ":" + self.TYPE_TIME_MS + ":Input"
            )
            tag_node_inputXX_value_name += (
                str(self._slot_id[tag_node_name]).zfill(2) + "Value"
            )
