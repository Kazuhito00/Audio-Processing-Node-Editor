...
class Node(DpgNodeABC):
    ...
    def __init__(self) -> None:
        ...
        self._default_input_id: int = sd.default.device[0]  # (input_id, output_id)

        # マイクリスト作成（[デバイス名, ID]のリスト）
        self._mic_list: List[Tuple[str, int]] = []
        for input_id, device in enumerate(sd.query_devices()):
            if device["max_input_channels"] > 0:
                if input_id == self._default_input_id:
                    self._mic_list.append([device["name"], input_id])
        for input_id, device in enumerate(sd.query_devices()):
            if device["max_input_channels"] > 0:
                if input_id != self._default_input_id:
                    self._mic_list.append([device["name"], input_id])
    ...
    def add_node(
        ...
        self._node_data[str(node_id)] = {
            ...
            "input_id": self._default_input_id,
        }
        ...
        with dpg.node(
            ...
            with dpg.node_attribute(
                ...
                mic_names: List[str] = [name for name, _ in self._mic_list]
                dpg.add_combo(
                    mic_names,
    ...
    def update(
        ...
        if current_status == "play":
            # マイク開始
            if "stream" not in self._node_data[str(node_id)]:
                ...
                self._node_data[str(node_id)]["stream"] = sd.InputStream(
                    samplerate=self._default_sampling_rate,
                    channels=1,
                    blocksize=self._chunk_size,
                    device=self._node_data[str(node_id)]["input_id"],
                    dtype="float32",
            ...
            # マイク終了
            self._node_data[str(node_id)]["buffer"] = np.zeros(0, dtype=np.float32)
            if "stream" in self._node_data[str(node_id)]:
                self._node_data[str(node_id)]["stream"].close()
                del self._node_data[str(node_id)]["stream"]
    ...
    def _on_mic_select(self, sender, app_data, user_data):
        ...
        for device_name, input_id in self._mic_list:
            if app_data == device_name:
                self._node_data[str(node_id)]["input_id"] = input_id
