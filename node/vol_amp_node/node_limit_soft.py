...
class Node(DpgNodeABC):
    ...
    node_label = "Soft Limit(tanh)"
    node_tag = "SoftLimitTanh"
    ...
    def update(
        ...
        gain = float(dpg_get_value(input_tag_list[1][1]))
        gain = float(np.clip(gain, 0.0, 20.0))
        dpg_set_value(input_tag_list[1][1], gain)
        ...
        if current_status == "play" and chunk_index >= 0 and len(chunk) > 0:
            ...
            if self._node_data[str(node_id)]["current_chunk_index"] < chunk_index:
                if (
                    ...
                    print(
                        f"    [Warning] Soft Limit Node Chunk Index Gap: {chunk_index - self._node_data[str(node_id)]['current_chunk_index']} (Index: {self._node_data[str(node_id)]['current_chunk_index']} -> {chunk_index})"
                    )

                # チャンク生成
                scale_chunk = np.tanh(chunk * gain)
    ...
    def get_setting_dict(self, node_id):
        ...
        gain: float = float(dpg_get_value(input_tag_list[1][1]))
