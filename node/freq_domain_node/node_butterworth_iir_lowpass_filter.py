...
class Node(DpgNodeABC):
    ...
    node_label = "Lowpass Filter(Butterworth IIR)"
    node_tag = "LowpassFilter"
    ...
    def add_node(
        ...
        tag_name_list: List[Any] = get_tag_name_list(
            node_id,
            self.node_tag,
            [self.TYPE_SIGNAL_CHUNK, self.TYPE_INT, self.TYPE_INT],
    ...
    def update(
        ...
        # フィルター設定値
        lowcut = 0
        highcut = int(dpg_get_value(input_tag_list[1][1]))
        order = int(dpg_get_value(input_tag_list[2][1]))
        if highcut >= int(self._default_sampling_rate / 2):
            highcut = int(self._default_sampling_rate / 2)
            dpg_set_value(input_tag_list[1][1], highcut)
        if highcut <= 0:
            highcut = 1
            dpg_set_value(input_tag_list[1][1], highcut)
    ...
    def set_setting_dict(self, node_id, setting_dict):
        ...
        dpg_set_value(input_tag_list[1][1], highcut)
        dpg_set_value(input_tag_list[2][1], order)
