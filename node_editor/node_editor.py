...
class DpgNodeEditor(object):
    ...
    def __init__(
        ...
        node_dir: str = "node",
        menu_dict: Optional[Dict[str, str]] = None,
        use_debug_print: bool = False,
    ) -> None:
        # 各種初期化
        self._node_id = 0
        self._node_instance_list: Dict[str, Any] = {}
        self._node_list: List[str] = []
        self._node_link_list: List[List[str]] = []
        self._node_connection_dict: Dict[str, List[List[str]]] = OrderedDict()
        self._use_debug_print = use_debug_print
        self._terminate_flag = False
        self._setting_dict = setting_dict if setting_dict is not None else {}
        self._last_pos: Optional[List[int]] = None
    ...
    def get_sorted_node_connection(self) -> Dict[str, List[List[str]]]:
        return self._node_connection_dict

    def get_node_instance(self, node_name: str) -> Optional[Any]:
    ...
    def add_node(self, user_data: str) -> None:
        self._node_id += 1
        ...
        if self._last_pos is not None:
            last_pos = [self._last_pos[0] + 30, self._last_pos[1] + 60]
    ...
    def _callback_add_node(self, sender: Any, data: Any, user_data: str) -> None:
        if self._use_debug_print:
        ...
        self.add_node(user_data)

    def _callback_link(self, sender: Any, data: Tuple[int, int]) -> None:
        # 各接続子の型を取得
    ...
    def _callback_close_window(self, sender: Any) -> None:
        dpg.delete_item(sender)

    def _sort_node_graph(
        self, node_list: List[str], node_link_list: List[List[str]]
    ) -> Dict[str, List[List[str]]]:
        node_id_dict: Dict[int, List[int]] = OrderedDict()
        node_connection_dict: Dict[str, List[List[str]]] = OrderedDict()
    ...
    def _callback_file_export(self, sender: Any, data: Dict[str, Any]) -> None:
        # ノードリスト、接続リスト保存
        setting_dict: Dict[str, Any] = {
            "node_list": self._node_list,
            "link_list": self._node_link_list,
        }
    ...
    def _callback_file_export_menu(self) -> None:
        dpg.show_item("file_export")

    def _callback_file_import_menu(self) -> None:
        if self._node_id == 0:
            dpg.show_item("file_import")
        else:
            dpg.configure_item("modal_file_import", show=True)

    def _callback_file_import(self, sender: Any, data: Dict[str, Any]) -> None:
        if data["file_name"] != ".":
            # JSONファイルから読み込み
            setting_dict = None
    ...
    def _callback_save_last_pos(self) -> None:
    ...
    def _callback_mv_key_del(self) -> None:
        if len(dpg.get_selected_nodes(self._node_editor_tag)) > 0:
            # 選択中のノードのアイテムIDを取得
